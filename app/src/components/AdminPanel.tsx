 ...
 * ─────────────────────────────────────────────────────────
 * Changes vs original:
 *  1. All CRUD now calls Supabase directly via supabaseClient helpers
 *     (no more mock in-memory state — data survives page refresh)
 *  2. Image inputs replaced with <ImageUploader> component that
 *     uploads to Supabase Storage bucket "yy-images" and returns
 *     a public URL — no more pasting Unsplash links.
 *  3. Supabase tab shows live row counts pulled from the DB.
 *  4. Pre-orders feature removed.
 *  5. Top Slider / Hero Slider / Brand CMS merged into the Promos tab.
 *
...
interface ImageUploaderProps {
  ...
  bucket?: string;                  // default: 'yy-images'
  folder?: string;                  // e.g. 'products' | 'banners'
  label?: string;
}
...
// All data lives in yy_store_sync(key text PK, value jsonb, updated_at timestamptz)
...
export const AdminPanel: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'products' | 'orders' | 'new-orders' | 'promos' | 'supabase'>('dashboard');
  ...
  const loadAll = useCallback(async () => {
    ...
    // Check if the key exists to distinguish empty categories vs unset categories
  ...
  useEffect(() => {
    ...
    // Process real data into chart points based on chartTimeframe
    const now = new Date();
    ...
    let revenueData: number[] = [];
      ...
      // past 6 months for 1M (or 30 days)
      // let's do past 30 days every 5 days or past 12 months, usually 1M is past 30 days. Let's do past 30 days grouped by day
      for (let i = 29; i >= 0; i--) {
        const d = new Date(now.getTime() - i * 24 * 60 * 60 * 1000);
        // Only push label every 5 days for clarity, except last
    ...
    // Removed fake fallback, 100% real data
    const maxRev = Math.max(...revenueData, 1000); // ensure chart doesn't collapse if all 0
        ...
        datasets: [
          {
            ...
            fill: true,
            tension: 0, // 0 for jagged lines like Google Finance
            yAxisID: 'y'
          }
        ]
      },
  ...
  const handleTriggerAddProduct = () => {
    ...
    setPMRP(undefined); setPWeight(1); setPSizePrices({}); setPSizeMRPs({}); setPSizeWeights({});
    setShowProductModal(true);
  };
  ...
  const handleUpdateOrderBuyback = async (id: string, status: 'Confirmed' | 'Rejected') => {
    const updated = orders.map(o => {
      if (o.id === id && o.buyback_requested && o.buyback_details) {
        ...
        if (status === 'Rejected' && o.buyback_details.status !== 'Rejected') {
          newTotal = Math.round(o.total / 0.9); // Revert the 10% discount
        } else if (status === 'Confirmed' && o.buyback_details.status === 'Rejected') {
          newTotal = Math.round(o.total * 0.9); // Apply discount back if changed
        }
        return {
          ...o,
          total: newTotal,
          buyback_details: { ...o.buyback_details, status }
        };
      }
      return o;
    });
  ...
  return (
    <>
      <style>{`
        ...
        .nav { display:flex;align-items:center;gap:9px;padding:8px 10px;border-radius:var(--border-radius-md);cursor:pointer;font-size:12px;color:var(--color-text-secondary);transition:background 0.12s; }
        ...
        /* ── TOPBAR ── */
        .topbar { display:flex;align-items:center;justify-content:space-between;padding:12px 16px;border-bottom:0.5px solid var(--color-border-tertiary);background:var(--color-background-secondary);gap:8px;flex-wrap:wrap; }
        .pg-title { font-size:13px;font-weight:600;color:var(--color-text-primary); }
        .pg-sub { font-size:10px;color:var(--color-text-tertiary);margin-top:1px; }
        .tb-right { display:flex;align-items:center;gap:6px;flex-shrink:0; }
        .tb-btn { background:var(--color-background-secondary);border:0.5px solid var(--color-border-secondary);border-radius:var(--border-radius-md);padding:6px 10px;font-size:11px;color:var(--color-text-secondary);cursor:pointer;display:flex;align-items:center;gap:4px;transition:0.12s;white-space:nowrap; }
        .tb-btn:hover { background:var(--color-background-tertiary); }
        .tb-btn.gold { background:rgba(184,144,42,0.1);border-color:rgba(184,144,42,0.35);color:#b8902a; }
        .tb-btn.gold:hover { background:rgba(184,144,42,0.15); }
        ...
        .bb-thumb { width:30px;height:30px;border-radius:6px;background:var(--color-background-tertiary);border:0.5px solid var(--color-border-tertiary);display:flex;align-items:center;justify-content:center; }
        .bb-nm{font-size:11px;color:var(--color-text-primary);}
        .bb-cond{font-size:9px;color:var(--color-text-tertiary);margin-top:2px;}
        .bb-amt{font-size:11px;color:var(--color-text-secondary);}
        .bb-btns{display:flex;gap:4px;}
        .ab{width:22px;height:22px;border-radius:5px;border:none;cursor:pointer;display:flex;align-items:center;justify-content:center;}
        .ab.ok{background:rgba(45,143,90,0.1);color:#2d8f5a;}
        ...
        @media (max-width: 767px) {
          .adm { flex-direction: column; min-height: auto; margin-bottom: 72px; }
          .sb { display: none; }
          .mob-nav { display: flex; z-index: 80; }
          .main { flex: 1; overflow: visible; padding-bottom: 80px; }
          .topbar { padding: 10px 14px; }
          .pg-sub { display: none; }
          .tb-btn:not(.gold):not(.av-wrap) { display: none; }
      ...
      <div className="min-h-screen bg-neutral-900 pt-20 sm:pt-32 pb-0 font-sans select-none">
        <div className="adm w-full max-w-[1280px] mx-auto">

          {/* ── DESKTOP SIDEBAR ── */}
          <div className="sb">
            <div className="sb-logo">
              <div className="sb-brand">YY Leathers</div>
          ...
          <nav className="mob-nav">
            <div className="mob-nav-inner">
              {([
                ...
                <div
                  key={key}
                  className={`mob-tab ${activeTab === key ? 'on' : ''}`}
                  onClick={() => setActiveTab(key as any)}
                >
                  {icon}
                  <span>{label}</span>
                  {badge !== undefined && badge > 0 && (
                    <span className="mob-tab-badge">{badge}</span>
                  )}
                </div>
              ))}
            </div>
          ...
          <div className="main">
            <div className="topbar">
              <div className="min-w-0">
                <div className="pg-title">Welcome back, AFROZ 👋</div>
                <div className="pg-sub">{new Intl.DateTimeFormat('en-US', { weekday: 'long', day: 'numeric', month: 'long', year: 'numeric' }).format(new Date())} — Chennai Atelier</div>
              </div>
              <div className="tb-right">
                <div className="tb-btn" onClick={() => window.open('/', '_blank')}><Sparkles className="w-3 h-3" /><span className="hidden sm:inline">Live Shop</span></div>
                <div className="tb-btn" onClick={loadAll}><RefreshCw className="w-3 h-3" /><span className="hidden sm:inline">Refresh</span></div>
                <div className="tb-btn gold" onClick={handleTriggerAddProduct}><Plus className="w-3 h-3" /><span className="hidden sm:inline">Add product</span><span className="sm:hidden">Add</span></div>
            ...
            <div className="content adm-scroll">
                    ...
                    <div className="panel flex flex-col min-h-[290px]">
                      ...
                      <div className="flex-1 overflow-y-auto adm-scroll">
                        {filteredOrders.slice(0, 10).map((o, idx) => (
                          <div key={idx} className="or">
                            <div className="o-id">#ORD-{(o.id || '').substring(0, 4).toUpperCase()}</div>
                            <div className="o-cust">{o.customer_name || 'Guest'} · {(o.address || '').substring(0, 10)}…</div>
                      ...
                      <div key={ord.id} className="bg-white border rounded-xl overflow-hidden shadow-sm">
                        ...
                        <div className="p-4 flex flex-col md:flex-row gap-4 justify-between items-start md:items-center">
                              ...
                              <div className="mt-3 p-3 bg-gold/5 border border-gold/20 rounded text-xs space-y-1.5">
                                ...
                                <div className="mt-2 flex items-center justify-between pt-2 border-t border-gold/10">
                                  ...
                                  {ord.buyback_details.status === 'Pending' && (
                                    <div className="flex gap-2">
                                      <button onClick={() => handleUpdateOrderBuyback(ord.id, 'Confirmed')} className="px-2 py-1 bg-green-500 hover:bg-green-600 text-white rounded text-[10px] font-bold uppercase transition-colors cursor-pointer">Confirm Offer</button>
                                      <button onClick={() => handleUpdateOrderBuyback(ord.id, 'Rejected')} className="px-2 py-1 bg-red-500 hover:bg-red-600 text-white rounded text-[10px] font-bold uppercase transition-colors cursor-pointer">Reject</button>
                                    </div>
                                  )}
                                  {ord.buyback_details.status === 'Confirmed' && (
                                    <p className="font-bold text-green-600 text-[10px] uppercase">10% Off Applicable</p>
                                  )}
                                </div>
                              </div>
                            )}
                          ...
                          <div className="flex flex-wrap gap-2">
                            {(['Pending', 'Confirmed', 'Dispatched', 'Delivered', 'Cancelled'] as const).map((step) => (
                      ...
                      <div key={ord.id} className="p-5 border rounded-xl space-y-4 bg-white shadow-sm">
                          ...
                          <div>
                            ...
                            <div className="mt-2 text-neutral-500">
                              {ord.items.map((item, idx) => (
                                <p key={idx} className="mb-0.5 whitespace-nowrap overflow-hidden text-ellipsis">
                          ...
                          <div className="mt-2 p-3 bg-gold/5 border border-gold/20 rounded text-xs space-y-1.5">
                            ...
                            <div className="mt-2 flex items-center justify-between pt-2 border-t border-gold/10">
                              ...
                              {ord.buyback_details.status === 'Pending' && (
                                <div className="flex gap-2">
                                  <button onClick={() => handleUpdateOrderBuyback(ord.id, 'Confirmed')} className="px-2 py-1 bg-green-500 hover:bg-green-600 text-white rounded text-[10px] font-bold uppercase transition-colors cursor-pointer">Confirm Offer</button>
                                  <button onClick={() => handleUpdateOrderBuyback(ord.id, 'Rejected')} className="px-2 py-1 bg-red-500 hover:bg-red-600 text-white rounded text-[10px] font-bold uppercase transition-colors cursor-pointer">Cancel Offer</button>
                                </div>
                              )}
                              {ord.buyback_details.status === 'Confirmed' && (
                                <p className="font-bold text-green-600 text-[10px] uppercase">10% Off Applicable</p>
                        ...
                        <div className="bg-neutral-50 p-4 rounded-lg flex flex-col md:flex-row gap-3 items-end text-xs mt-3">
                          <div className="flex gap-2 ml-auto w-full md:w-auto mt-2 md:mt-0">
                            ...
                            <button onClick={() => handleUpdateOrderStatus(ord.id, 'Cancelled')} className="flex-1 md:flex-none justify-center bg-red-600 hover:bg-red-700 text-white font-bold py-2 px-6 rounded text-[10px] uppercase cursor-pointer flex items-center gap-2"><X className="w-3.5 h-3.5" /> Decline Order</button>
                          </div>
                        </div>
              ...
              {/* ── PROMOS (Offers & Banners + Top Slider + Hero Slider + Brand CMS) ── */}
              {!loadingData && activeTab === 'promos' && (
                <div className="space-y-10">
                  ...
                  <div className="space-y-6 pt-8 border-t">
                    ...
                    <form onSubmit={handleAddTopSliderMsg} className="bg-neutral-50 p-5 rounded-xl border flex gap-4 text-xs items-end">
                      <div className="flex-1">
                        ...
                        <input 
                          type="text" 
                          value={newTopSliderMsg} 
                          onChange={(e) => setNewTopSliderMsg(e.target.value)} 
                          placeholder="e.g. GET EXTRA 10% OFF ON PREPAID ORDERS"
                          className="w-full p-2.5 border rounded bg-white focus:outline-none focus:border-leather" 
                        />
                      </div>
                  ...
                  <div className="space-y-6 pt-8 border-t">
                    ...
                    <div className="space-y-3">
                      <h4 className="text-xs uppercase tracking-wider font-bold text-neutral-400">Published Slides</h4>
                      {!heroSlides || heroSlides.length === 0 ? <p className="text-xs text-neutral-400 italic text-center py-6">No slides configured.</p> : heroSlides.map((slide) => (
                        <div key={slide.id} className="flex gap-4 p-4 border rounded-xl bg-white items-center">
            ...
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 20 }}
              ...
              <form onSubmit={handleSaveProduct} className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
                <div className="space-y-3">
                  ...
                  <div>
                    <label className="block font-semibold mb-1 text-neutral-700">Sizes, price, MRP &amp; weight</label>
                    <div className="grid grid-cols-2 gap-2">
                    ...
                    <p className="text-[11px] text-neutral-400 mt-2">Toggle a size to add a custom price, MRP, and weight. Leave blank to use the base values.</p>
                  </div>
