...
type AppliedOffer = 'none' | 'student' | 'birthday' | 'buyback';

export const Checkout: React.FC = () => {
  const { cart, user, navigateTo, clearCart } = useApp();
  ...
  const requiresManualQuote = selectedRegion === 'Others';

  // ----- OFFER LOGIC: Only ONE offer can apply -----
  
  const [appliedOffer, setAppliedOffer] = useState<AppliedOffer>('none');
  const [selectedBuybackForCheckout, setSelectedBuybackForCheckout] = useState(false);

  // Reset dependent offers when one is selected
  ...
  const buybackDiscountAmount = selectedBuybackForCheckout ? Math.min(subtotal * 0.1, 500) : 0;
  
  // Total discount (only ONE will be non-zero due to appliedOffer logic)
  const totalOfferDiscount = studentDiscountAmount + birthdayDiscount + buybackDiscountAmount;
  const orderTotal = Math.max(0, subtotal + processingFee + deliveryCharge - totalOfferDiscount);
  ...
  const handleCheckoutSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store the order body for saving after payment
      setOrderBody(body);
  ...
  if (orderSuccess) {
    return (
      <div id="checkout-completed-stage" className="min-h-screen bg-neutral-50 pt-36 pb-24 px-4 flex items-center justify-center">
        ...
        >
          <div className="w-16 h-16 bg-green-50 text-green-500 border border-green-200 rounded-full flex items-center justify-center text-3xl font-bold mx-auto animate-pulse">
            ✓
          </div>
          
          <h2 className="font-serif text-3xl font-bold text-neutral-800">
            Order Confirmed!
          </h2>
          <p className="text-xs text-gold uppercase tracking-[0.25em] font-bold">
            Razorpay Payment ID: {orderId}
          </p>
          ...
          <div className="bg-neutral-50 p-5 rounded-lg border border-neutral-100 text-left text-xs font-sans text-neutral-600 space-y-2 max-w-md mx-auto">
            ...
            {appliedOffer !== 'none' && (
              <p className="text-green-700 font-semibold">
                ✓ {appliedOffer === 'student' ? 'Student Discount' : appliedOffer === 'birthday' ? 'Birthday Benefit' : 'Buyback Benefit'} Applied
              </p>
            )}
            <p className="text-[10px] text-neutral-450 leading-relaxed pt-2.5">
              You will receive an email confirmation shortly. Thank you for your purchase from YY Leathers.
            </p>
          </div>
          ...
          <div className="flex gap-4 justify-center pt-4">
            <button
              ...
              className="bg-leather text-white font-sans text-xs uppercase tracking-widest font-bold py-3.5 px-6 rounded hover:bg-gold transition-colors cursor-pointer"
            >
              Track Order
            </button>
            <button
              onClick={() => navigateTo('shop')}
              className="border border-neutral-300 text-neutral-700 font-sans text-xs uppercase tracking-widest font-semibold py-3.5 px-6 rounded hover:bg-neutral-50 transition-colors cursor-pointer"
            >
              Continue Shopping
            </button>
          </div>
      ...
      <div className="w-full">
        ...
        <div className="text-center space-y-2 mb-12">
          <span className="text-gold text-xs tracking-[0.3em] font-sans font-bold uppercase block col-span-2">
            SECURE ROYAL BILLING PORTAL
          </span>
          <h1 className="font-serif text-3xl sm:text-5xl font-bold tracking-wide text-leather">
            Handcrafted checkout
          </h1>
        </div>

        {checkoutError && (
          <div className="bg-red-50 border border-red-200 text-red-700 p-3 rounded-lg text-center text-xs mb-8 max-w-lg mx-auto">
            {checkoutError}
          </div>
        )}

        {/* OFFER SELECTION BANNER - Only One Applies */}
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 mb-8 max-w-4xl mx-auto">
          <div className="flex items-start gap-3">
            ...
            <div className="text-xs text-amber-800 font-sans">
              <p className="font-bold mb-1">🎯 Only One Offer Can Be Applied Per Order</p>
              <p>You can select only one of the following: <strong>Student Discount (₹100 OFF)</strong>, <strong>Birthday Benefit (₹250 OFF)</strong>, or <strong>Buyback Benefit (10% up to ₹500)</strong>. Choose wisely!</p>
            </div>
          </div>
        </div>
        ...
        {cart.length === 0 ? (
          <div className="bg-white border p-12 text-center rounded-2xl max-w-md mx-auto space-y-6">
            ...
            <button
              onClick={() => navigateTo('shop')}
              className="bg-leather text-white text-xs uppercase font-sans tracking-widest font-bold py-3 px-6 rounded hover:bg-gold transition-all"
            >
              Savor the Catalog
            </button>
          </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            {/* 1. Left Section: Profile and Address input Form (7 cols) */}
            <form onSubmit={handleCheckoutSubmit} className="lg:col-span-7 bg-white border border-neutral-200 rounded-2xl shadow-md overflow-hidden">
              <div className="bg-leather-dark text-white p-5 border-b border-gold/30 flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-[10px] text-neutral-300 font-sans uppercase">Aromatic Insured Freight Coverage</p>
                </div>
              </div>
              ...
              <div className="p-6 space-y-4 font-sans">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="space-y-1 text-xs">
                    ...
                    <input
                      type="text"
                      disabled
                      value={user?.name || 'Inquire login guest'}
                      className="w-full p-3 bg-neutral-55 border border-neutral-150 text-neutral-450 rounded-lg cursor-not-allowed uppercase text-[11px]"
                    />
                  </div>
                  <div className="space-y-1 text-xs">
                    ...
                    <input
                      ...
                      disabled
                      value={user?.email || ''}
                      className="w-full p-3 bg-neutral-55 border border-neutral-150 text-neutral-450 rounded-lg cursor-not-allowed text-[11px]"
                    />
                  </div>
                </div>
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <input
                    ...
                    placeholder="Provide Chennai or national mobile coord (e.g., +91 98400 12345)"
                ...
                <div className="space-y-1 text-xs">
                  ...
                  <textarea
                    ...
                    placeholder="State Gated apartments, Door levels, streets, City, State, and ZIP PIN code..."
                    className="w-full p-3 border border-neutral-200 rounded-lg focus:outline-none focus:border-gold text-xs leading-relaxed"
                  />
                </div>
                  ...
                  >
                    {deliveryRates.map((item) => (
                      <option key={item.region} value={item.region}>
                        {item.region} {typeof item.rate === 'number' ? `(₹${item.rate}/Kg)` : `(${item.rate})`}
                      </option>
                    ))}
                  </select>
                  {requiresManualQuote ? (
                    <p className="text-[10px] text-amber-700 bg-amber-50 border border-amber-200 rounded p-2 mt-2">
                      Please contact us for shipping rates for this region.
                    </p>
                  ) : (
                ...
                {/* ============ OFFER SECTION: Only One Applies ============ */}
                
                {/* --- OFFER 1: BUYBACK --- */}
                <div className="space-y-3 bg-white p-3 rounded border border-amber-200 shadow-sm mt-3 font-sans">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-bold text-neutral-600 uppercase tracking-wide flex items-center gap-2">
                      <RefreshCw className="w-3.5 h-3.5 text-amber-600" />
                      Buyback / Upgrade Benefit (10% up to ₹500)
                    </label>
                    <div className="flex bg-neutral-100 rounded p-1">
                      <button
                        type="button"
                        onClick={() => handleBuybackToggle(true)}
                        className={`px-3 py-1 text-[10px] font-bold rounded transition-colors ${selectedBuybackForCheckout ? 'bg-amber-600 text-white' : 'text-neutral-500 hover:text-neutral-800'}`}
                        disabled={includeStudentDiscount || includeBirthdayBenefit}
                      >
                        YES
                      </button>
                      <button
                        type="button"
                        onClick={() => handleBuybackToggle(false)}
                        className={`px-3 py-1 text-[10px] font-bold rounded transition-colors ${!selectedBuybackForCheckout ? 'bg-neutral-500 text-white' : 'text-neutral-500 hover:text-neutral-800'}`}
                      >
                        NO
                      </button>
                    </div>
                  ...
                  {selectedBuybackForCheckout && (
                    <div className="text-[10px] text-amber-700 bg-amber-50 border border-amber-200 rounded p-2">
                      Buyback benefit of ₹{buybackDiscountAmount.toLocaleString('en-IN')} (10% up to ₹500) will be applied. You will need to surrender old YY Leathers footwear post-purchase.
                    </div>
                  )}
                </div>

                {/* --- OFFER 2: STUDENT DISCOUNT --- */}
                <div className="space-y-3 bg-white p-3 rounded border border-blue-200 shadow-sm mt-3 font-sans">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-bold text-neutral-600 uppercase tracking-wide flex items-center gap-2">
                      <GraduationCap className="w-3.5 h-3.5 text-blue-600" />
                      Student Discount? (Flat ₹100 OFF)
                    </label>
                    <div className="flex bg-neutral-100 rounded p-1">
                      <button
                        type="button"
                        onClick={() => handleStudentToggle(true)}
                        className={`px-3 py-1 text-[10px] font-bold rounded transition-colors ${includeStudentDiscount ? 'bg-blue-600 text-white' : 'text-neutral-500 hover:text-neutral-800'}`}
                        disabled={includeBirthdayBenefit || selectedBuybackForCheckout}
                      >
                        YES
                      </button>
                      <button
                        ...
                        className={`px-3 py-1 text-[10px] font-bold rounded transition-colors ${!includeStudentDiscount ? 'bg-neutral-500 text-white' : 'text-neutral-500 hover:text-neutral-800'}`}
                      >
                        NO
                      </button>
                    </div>
                  ...
                  {includeStudentDiscount && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 pt-2 border-t border-dashed border-neutral-200 p-1">
                      <div className="sm:col-span-2">
                        <input type="text" value={studentCollegeName} onChange={(e) => setStudentCollegeName(e.target.value)} required placeholder="College / University Name" className="w-full text-xs p-2 border rounded bg-white text-neutral-800 focus:border-blue-500" />
                      </div>
                      <div>
                        <input type="text" value={studentIdNumber} onChange={(e) => setStudentIdNumber(e.target.value)} required placeholder="Student ID Number" className="w-full text-xs p-2 border rounded bg-white text-neutral-800 focus:border-blue-500" />
                      </div>
                      <div>
                        <div className="flex items-center gap-2">
                          <span className="text-[10px] font-semibold text-neutral-500 whitespace-nowrap">Upload College ID:</span>
                          <input type="file" accept="image/*" onChange={handleStudentPhotoChange} required className="w-full text-xs p-2 border rounded bg-white text-neutral-800 focus:border-blue-500" />
                        </div>
                      </div>
                      {studentIdPhotoUrl && (
                        <div className="sm:col-span-2">
                          <img src={studentIdPhotoUrl} alt="Student ID" className="mt-1 h-24 w-full object-cover rounded border border-neutral-200" />
                        </div>
                      )}
                      <p className="sm:col-span-2 text-[9px] text-neutral-500 italic">Your student discount request will be saved for admin review and verification. Only one offer can be applied per order.</p>
                    </div>
                  )}
                </div>

                {/* --- OFFER 3: BIRTHDAY BENEFIT --- */}
                <div className="space-y-3 bg-white p-3 rounded border border-pink-200 shadow-sm mt-3 font-sans">
                  <div className="flex items-center justify-between">
                    <label className="text-[10px] font-bold text-neutral-600 uppercase tracking-wide flex items-center gap-2">
                      <Gift className="w-3.5 h-3.5 text-pink-600" />
                      Birthday Benefit? (Flat ₹250 OFF Online)
                    </label>
                    <div className="flex bg-neutral-100 rounded p-1">
                      <button
                        type="button"
                        onClick={() => handleBirthdayToggle(true)}
                        className={`px-3 py-1 text-[10px] font-bold rounded transition-colors ${includeBirthdayBenefit ? 'bg-pink-600 text-white' : 'text-neutral-500 hover:text-neutral-800'}`}
                        disabled={includeStudentDiscount || selectedBuybackForCheckout}
                      >
                        YES
                      </button>
                      <button
                        type="button"
                        onClick={() => handleBirthdayToggle(false)}
                        className={`px-3 py-1 text-[10px] font-bold rounded transition-colors ${!includeBirthdayBenefit ? 'bg-neutral-500 text-white' : 'text-neutral-500 hover:text-neutral-800'}`}
                      >
                        NO
                      </button>
                    </div>
                  ...
                  {includeBirthdayBenefit && (
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-2 mt-2 pt-2 border-t border-dashed border-neutral-200 p-1">
                      <div>
                        <input type="text" value={birthdayGovIdNumber} onChange={(e) => setBirthdayGovIdNumber(e.target.value)} required placeholder="Government ID number" className="w-full text-xs p-2 border rounded bg-white text-neutral-800 focus:border-pink-500" />
                      </div>
                      <div>
                        <input type="date" value={birthdayDob} onChange={(e) => setBirthdayDob(e.target.value)} required title="Date of birth" className="w-full text-xs p-2 border rounded bg-white text-neutral-800 focus:border-pink-500" />
                      </div>
                      <div className="sm:col-span-2">
                        ...
                        {birthdayGovIdPhotoUrl && (
                          <img src={birthdayGovIdPhotoUrl} alt="Selected government ID" className="mt-2 h-24 w-full object-cover rounded border border-neutral-200" />
                        )}
                      </div>
                      <p className="sm:col-span-2 text-[9px] text-neutral-500 italic">Your birthday benefit request will be saved for admin review and verification. Only one offer can be applied per order.</p>
                    </div>
                  )}
                </div>
                ...
                <div className="bg-gold/5 border border-gold/15 p-4 rounded-lg flex items-center gap-3">
                  ...
                  <div className="text-[11px] text-neutral-600 leading-relaxed">
                    ...
                    {razorpayKeyId ? (
                      <span className="block mt-1 text-[10px] text-green-700">Razorpay is configured for payments.</span>
                    ) : (
                      <span className="block mt-1 text-[10px] text-amber-700">Razorpay key is not configured yet.</span>
                    )}
                  </div>
                </div>
              ...
              {isPaymentReady && razorpayOrderId ? (
                ...
                <span className="text-[10px] text-neutral-450 font-sans tracking-wide">
                  ※ High-Arch premium welt adjustments included on dispatch.
                </span>
                
                <button
                  type="submit"
                  disabled={isAssemblingGateway || requiresManualQuote}
            ...
            {/* 2. Right Section: Summary of invoice (5 cols) */}
            <div id="checkout-invoice-summary" className="lg:col-span-12 xl:col-span-5 bg-white border border-neutral-200 rounded-2xl shadow-md p-6 space-y-6">
              <h3 className="font-serif text-lg font-bold border-b border-neutral-100 pb-3 text-neutral-800">
                Your Luxury Trunk Details
              </h3>
                ...
                {cart.map((item, id) => (
                  <div key={id} className="flex items-center gap-4 text-xs">
                    <img 
                      ...
                      alt="" 
                      className="w-12 h-12 object-cover rounded border border-neutral-100"
                    />
                    <div className="flex-1 min-w-0">
                      <p className="font-serif font-bold text-neutral-800 truncate">{item.product.name}</p>
                      <p className="text-[10px] text-neutral-400 font-sans tracking-widest uppercase">
              ...
              <div className="border-t border-neutral-100 pt-4 space-y-3 font-sans text-xs">
                ...
                <div className="flex justify-between text-neutral-500">
                  ...
                  <span className="font-bold text-neutral-800">
                    {requiresManualQuote ? 'Contact' : `₹${deliveryCharge.toLocaleString('en-IN')}`}
                  </span>
                </div>

                {/* Offer discount lines - only show the applied one */}
                {includeStudentDiscount && (
                  <div className="flex justify-between text-blue-600">
                    <span>🎓 Student Discount</span>
                    <span className="font-bold">-₹{studentDiscountAmount.toLocaleString('en-IN')}</span>
                  </div>
                )}

                {includeBirthdayBenefit && (
                  <div className="flex justify-between text-pink-600">
                    <span>🎂 Birthday Benefit Discount</span>
                    <span className="font-bold">-₹{birthdayDiscount.toLocaleString('en-IN')}</span>
                  </div>
                )}

                {selectedBuybackForCheckout && (
                  <div className="flex justify-between text-amber-600">
                    <span>🔄 Buyback Benefit</span>
                ...
                <div className="border-t border-neutral-150 pt-3 flex justify-between items-center text-neutral-800 font-bold text-sm">
                  ...
                  <span className="text-lg text-leather text-black">
              ...
              <div className="bg-neutral-50 p-4 border rounded-xl flex items-center gap-3">
                ...
                <p className="text-[10px] text-neutral-450 leading-relaxed font-sans">
                  <strong>YY Lifetime authentic Guarantee.</strong> Sourced from zero-waste local leather tanneries in Chennai. Blake-welted by hand.
                </p>
              </div>
              ...
              <div className="bg-white border border-neutral-200 rounded-2xl p-4">
                ...
                <div className="grid grid-cols-2 gap-3 text-xs text-neutral-700">
                  {deliveryRates.map((item) => (
                    <div key={item.region} className="rounded-xl border border-neutral-150 p-3 bg-neutral-50">
                      <div className="font-semibold text-neutral-900">{item.region}</div>
                      <div className="mt-1 text-sm font-bold text-leather">
                        {typeof item.rate === 'number' ? `₹${item.rate}` : item.rate}
                      </div>
                    </div>
                  ))}
                </div>
                <p className="mt-3 text-[10px] text-neutral-500">Delivery is charged per kilogram. For rates outside listed regions, please contact customer support.</p>
              </div>
